...
namespace PIM_FINAL.Controllers
{
 ...
 new Claim(ClaimTypes.Role, user.PerfilId.Value.ToString())
            ...
            {
                TempData["ErrorMessage"] = "Banco de dados indisponível (timeout). Tente novamente em instantes.";
                return RedirectToAction("Page", new { viewName = "Login" });
            }
            catch (NpgsqlException)
            {
                TempData["ErrorMessage"] = "Falha ao conectar ao banco de dados. Verifique a conexão e tente novamente.";
                return RedirectToAction("Page", new { viewName = "Login" });
            }
            catch (InvalidOperationException)
            {
                TempData["ErrorMessage"] = "Serviço temporariamente indisponível. Tente novamente.";
                return RedirectToAction("Page", new { viewName = "Login" });
            }
        }
 ...
 return RedirectToAction("MeusChamados");
        ...
        public async Task<IActionResult> MeusChamados(string? status = null, string? prioridade = null, string? busca = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(busca))
            {
                busca = busca.Trim();
                query = query.Where(c =>
                    (c.Protocolo != null && c.Protocolo.Contains(busca, StringComparison.OrdinalIgnoreCase)) ||
                    (c.Titulo != null && c.Titulo.Contains(busca, StringComparison.OrdinalIgnoreCase)) ||
                    (c.Descricao != null && c.Descricao.Contains(busca, StringComparison.OrdinalIgnoreCase))
                );
            }
 ...
 $"SLACumpridoPercent,{sla}"
        ...
        private async Task<RelatoriosViewModel> BuildRelatoriosViewModel(string? periodo)
        {
            ...
            {
                var atendidos = await _db.Chamados.CountAsync(c => c.TecnicoResponsavelId == t.UsuarioId);
                var resolvT = await _db.Chamados.CountAsync(c => c.TecnicoResponsavelId == t.UsuarioId && c.DataFechamento != null);
        ...
        public async Task<IActionResult> ValidarCodigoRedefinir(string email, string novaSenha, string confirmaSenha)
        {
            ...
            if (string.IsNullOrWhiteSpace(novaSenha) || string.IsNullOrWhiteSpace(confirmaSenha)) { TempData["ErrorMessage"] = "Preencha a nova senha"; return RedirectToAction("ValidarCodigoRedefinirView", new { email }); }
